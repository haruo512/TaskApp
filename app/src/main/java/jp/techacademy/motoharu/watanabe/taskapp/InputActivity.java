...
//    private
    ...
    //Realmに保存・更新後、前の画面（MainActivity）に戻る
    ...
    private void addTask(){
        ...
        resultIntent.putExtra(MainActivity.EXTRA_TASK,mTask);           //タスクのタイトルなどの表示用
        PendingIntent resultPendingIntent = PendingIntent.getBroadcast(
                this,
                mTask.getId(),                      //タスク削除時に指定したアラームに合わせて削除する用
                resultIntent,
                PendingIntent.FLAG_UPDATE_CURRENT   //既存のPendingIntentがあれば、extraデータだけ置き換え
        );

        AlarmManager alarmManager = (AlarmManager)getSystemService(ALARM_SERVICE);
        //UTC時間を指定、スリープ中もアラート発行
